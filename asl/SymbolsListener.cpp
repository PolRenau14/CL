...
void SymbolsListener::exitFunction(AslParser::FunctionContext *ctx) {
  ...
  if (Symbols.findInCurrentScope(ident)) {
    ...
    for (auto declaration : ctx->decl()) {
        ...
        //AslParser::PairDeclContext* pairdeclaration = dynamic_cast<AslParser::PairDeclContext*>(declaration);
        ...
        /*
		else if (pairdeclaration) {
			TypesMgr::TypedId t0 = getTypeDecor(pairdeclaration->type(0));
			TypesMgr::TypedId t1 = getTypeDecor(pairdeclaration->type(1));
			t = Types.createPairTy(t0,t1);
		}
        */
        else {
...
void SymbolsListener::exitPairDecl(AslParser::PairDeclContext *ctx){
	...
	if(Symbols.findInCurrentScope(ident)){
      ...
      if (ctx->type(0)->INT()) {
          t0 = Types.createIntegerTy();
      }
      else if (ctx->type(0)->FLOAT()) {
          t0 = Types.createFloatTy();  
      } 
      else if (ctx->type(0)->BOOL()) {
          t0 = Types.createBooleanTy();
      }
      else if (ctx->type(0)->CHAR()) {
          t0 = Types.createCharacterTy();
      }

      (...) Tambien para type(1)
      TypesMgr::TypedId pair_type = Types.createPairTy(t0,t1);
      Symbols.addLocalVar(ident,pair_type);
	}
}
