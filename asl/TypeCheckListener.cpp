...
/*
void TypeCheckListener::exitPairLeftExpr(AslParser::PairLeftExprContext *ctx) {
	TypesMgr::TypedId t = getTypeDecor(ctx->pair:acces());
	putTypeDecor(ctx,t);
	putIsLValueDecor(ctx,true);
	DEBUG_EXIT();
}
...
/*
void TypeCheckListener::exitPair_acces(AslParser::Pair_accesContext *ctx) {
	TypesMgr::TypedId ident_type = getTypeDecor(ctx->ident());

	TypesMgr::TypeId error_type = Types.createErrorTy();
	if ( (not Types.isErrorTy(ident_type)) and (not Types.isPairTy(ident_type))) {
		Errors.nonPairInPairAcces(ctx);
		putTypeDecor(ctx, error_type);
		putIsLValueDecor(ctx, false);
	}
	else  {
		if( ctx->FIRST()) {
			TypesMgr::TypeId t = Types.getFirstPairType(ident_type);
		    putTypeDecor(ctx, t);
		    putIsLValueDecor(ctx, true);   
		}
		else {
			TypesMgr::TypeId t = Types.getSecondPairType(ident_type);
		    putTypeDecor(ctx, t);
		    putIsLValueDecor(ctx, true);   
		}
	}
...
void TypeCheckListener::exitPairExpr(AslParser::PairExprContext *ctx) {
  TypesMgr::TypeId t = getTypeDecor(ctx->pair_access());
  putTypeDecor(ctx, t);
  putIsLValueDecor(ctx, true); 
  DEBUG_EXIT();
}
